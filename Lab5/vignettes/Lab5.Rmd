...
This vignette shows how to use the UClassifier which is a wrapper around the rest-API provided by \url{uclassify.com}. The rest-API documentation can be found at their website and you need to create an account in order to use the classifier.

## Data

The first thing you need is data to classify. This package includes 'test_tweets' which is a data set of classified twitter tweets taken from \url{http://help.sentiment140.com/for-students}. The data can be loaded as follows:
...
In order to use textual data, you usually do some processing beforehand. I this case I use the tm library to do some standard operations.
...
The first step in training your classifier is to actually create it. Here I used environment variables to hide my private information. The username correspond to the username you registered with on uclassify.com and the read/write tokens can be found in your account settings under 'Api keys'.
...
With the data set ready for classification from previous step, I can use the classifier that was created. Before it is possible to train it, it is required to add classes for the classifier to recognize. In this case our classes are 'negative', 'neutral', and 'positive' so lets add them before training.
